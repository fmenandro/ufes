...
void elemento::draw(wxDC& dc, double xmed, double ymed, int d, double ddif, double zoom, double* view)
{
    ...
    int dim=qdim(), nn=qnno();
    dc.SetPen( wxPen( wxColor(0,0,0), 3 ) ); // black line, 3 pixels thick
//    dc.SetBrush( wxBrush(wxColour.MakeGrey(255,0,255,this->ro,0,1.0-this->ro)) );
    switch(dim)
    {
    case 1:
       for(int i=0;i<nn-1;i++)
    ...
    case 2:
        ...
        xm=ym=0.0;
            for(int i=0;i<nn-1;i++)
            {
                dc.DrawLine( w/2.+(view[0]*pno[i]->qx(0)+view[1]*pno[i]->qx(1)-xmed)*(zoom*d)/ddif,  
                             h/2.-(view[3]*pno[i]->qx(0)+view[4]*pno[i]->qx(1)-ymed)*(zoom*d)/ddif, 
                             w/2.+(view[0]*pno[i+1]->qx(0)+view[1]*pno[i+1]->qx(1)-xmed)*(zoom*d)/ddif,  
                             h/2.-(view[3]*pno[i+1]->qx(0)+view[4]*pno[i+1]->qx(1)-ymed)*(zoom*d)/ddif ); // draw line across the rectangle
                xm+=pno[i]->qx(0);
                ym+=pno[i]->qx(1);
            }
        xm/=nn;
        ym/=nn;
            dc.DrawLine( w/2.+(view[0]*pno[nn-1]->qx(0)+view[1]*pno[nn-1]->qx(1)-xmed)*(zoom*d)/ddif,  
                         h/2.-(view[3]*pno[nn-1]->qx(0)+view[4]*pno[nn-1]->qx(1)-ymed)*(zoom*d)/ddif, 
                         w/2.+(view[0]*pno[0]->qx(0)+view[1]*pno[0]->qx(1)-xmed)*(zoom*d)/ddif,  
                         h/2.-(view[3]*pno[0]->qx(0)+view[4]*pno[0]->qx(1)-ymed)*(zoom*d)/ddif ); // draw line across the rectangle
//            dc.FloodFill(w/2.+(view[0]*xm+view[1]*ym-xmed)*(zoom*d)/ddif,
//                         h/2.+(view[3]*xm+view[4]*ym-xmed)*(zoom*d)/ddif,
//                         dc.GetBrush(),wxBRUSHSTYLE_TRANSPARENT);
      break;
    }
}
